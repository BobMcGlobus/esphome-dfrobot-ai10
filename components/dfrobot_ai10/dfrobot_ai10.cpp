...
void DFRobotAI10Component::setup() {
  ...
  // Sensor needs 900ms–2.5s to boot (per datasheet)
  delay(3000);

  // Send reset command (mirrors Arduino library begin())
  this->send_reset();
    ...
    switch (this->parser_state_) {
      ...
      case STATE_READ_HEADER:
        ...
        if (this->header_pos_ >= 3) {
          ...
          if (this->payload_len_ > 280) {
            ...
            // Need to read payload_len_ data bytes + 1 XOR byte
      ...
      case STATE_READ_PAYLOAD:
        ...
        if (this->payload_pos_ >= this->payload_len_ + 1) {
          ...
          // XOR covers: header_buf_(3) + payload_buf_(payload_len_)
          uint8_t xor_calc = 0;
          ...
          if (xor_calc == xor_recv) {
            ...
            ESP_LOGI(TAG, "Packet received: MsgID=0x%02X, Len=%d, XOR OK", msg_id, this->payload_len_);
            this->log_hex_("  RX payload: ", this->payload_buf_, this->payload_len_);
            ...
            this->log_hex_("  Header: ", this->header_buf_, 3);
            this->log_hex_("  Payload: ", this->payload_buf_, this->payload_pos_);
          }
          this->parser_state_ = STATE_WAIT_SYNC_H;
        }
        break;
...
void DFRobotAI10Component::send_reset() {
  ...
  this->pending_cmd_ = CMD_RESET;
  ESP_LOGI(TAG, "CMD: RESET (0x10)");
}

void DFRobotAI10Component::start_recognition(uint8_t timeout, bool continuous) {
  // Reset first to abort any running operation (mirrors Arduino drvReset())
  this->send_reset();
  delay(100);
  uint8_t data[] = {
    CMD_VERIFY,
    0x00, 0x02,                          // Len = 2
    (uint8_t)(continuous ? 0x01 : 0x00), // continueVerify flag
    timeout
  };
  this->send_packet_(data, sizeof(data));
  this->pending_cmd_ = CMD_VERIFY;
  this->recognized_ = false;
  ESP_LOGI(TAG, "CMD: VERIFY (0x12) continuous=%d timeout=%ds", continuous, timeout);
}

void DFRobotAI10Component::enroll_user(uint8_t admin, const char *name, uint8_t timeout) {
  // Reset first to abort any running operation
  this->send_reset();
  delay(100);
  ESP_LOGI(TAG, "CMD: ENROLL '%s' admin=%d timeout=%ds", name, admin, timeout);

  // Build EnrollData struct (must match Arduino struct layout: 35 bytes total)
  EnrollData ed;
  memset(&ed, 0, sizeof(ed));
  ed.admin = admin;
  strncpy((char *)ed.user_name, name, 31);
  ed.face_dir = 0x00;  // FACE_DIRECTION_UNDEFINE
  ed.timeout = timeout;

  uint16_t struct_size = sizeof(EnrollData);  // 35

  // Packet data = CMD + LenH + LenL + struct bytes
...
void DFRobotAI10Component::delete_user(uint16_t uid) {
  ...
  ESP_LOGI(TAG, "CMD: DELETE USER uid=%d", uid);
}
...
void DFRobotAI10Component::scan_qr_code(uint8_t timeout) {
  this->send_reset();
  delay(100);
  uint8_t data[] = {CMD_SCAN_QR_CODE, 0x00, 0x01, timeout};
  this->send_packet_(data, sizeof(data));
  this->pending_cmd_ = CMD_SCAN_QR_CODE;
  ESP_LOGI(TAG, "CMD: SCAN QR CODE timeout=%ds", timeout);
}
...
void DFRobotAI10Component::send_packet_(const uint8_t *data, size_t len) {
  ...
  char tmp[4];
  sprintf(tmp, "%02X", xor_check);
  hex += tmp;
  ESP_LOGD(TAG, "TX: %s", hex.c_str());
}
...
void DFRobotAI10Component::handle_note_(const uint8_t *payload, uint16_t len) {
  ...
  if (len >= 17) {
    ...
    // Short note — state transition updates
